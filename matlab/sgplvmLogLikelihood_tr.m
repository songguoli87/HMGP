function ll = sgplvmLogLikelihood_tr(model,verbose)
...
% kernel F-norm constraints
if model.kernelFconstraints
  for(i = 1:1:model.numModels)
    model.comp{i}.mu = model.mu;
    ll = ll + kernelFconstraintLogLikelihood(model.comp{i});
  end
end
if(verbose)
  ll_part(9) = ll - sum(ll_part);
end

% kernel l21 norm constraints
if model.kernelSxL21norm  
  for(i = 1:1:model.numModels)
    model.comp{i}.SX = model.SX;
    model.comp{i}.mu = model.mu;
    [model.comp{i},ll_KSl21] = kernelSxL21normLogLikelihood(model.comp{i});
    ll = ll + ll_KSl21;
  end
end
if(verbose)
  ll_part(10) = ll - sum(ll_part);
end
...
if model.kernelSxtrace  
  for(i = 1:1:model.numModels)
    model.comp{i}.SX = model.SX;
    model.comp{i}.mu = model.mu;
    [model.comp{i},ll_invKS] = kernelSxtraceLogLikelihood(model.comp{i});
    ll = ll + ll_invKS;
  end
end
if(verbose)
