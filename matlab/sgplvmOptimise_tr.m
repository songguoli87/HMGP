function model = sgplvmOptimise_tr(model,display,iters,g_check,save_flag,options_sgplvm)
...
params = sgplvmExtractParam_tr(model);
options = optOptions;
if(display)
  options(1) = true;
  if(exist('g_check'))
...
elseif strcmp(func2str(optim), 'fmincon')
  ...
  if(isfield(model,'fols')&&~isempty(model.fols)&&model.fols.cropp&&exist('options_sgplvm','var'))
    ...
    for(i = 1:1:round(iters/model.fols.cropp_iter))
      ...
      sgplvmLogLikelihood_tr(model,display);
      sgplvmLogLikeGradients_tr(model,display);
      params = optim('sgplvmObjective', params, options, 'sgplvmGradient',model);
      model = sgplvmExpandParam_tr(model, params);
