function model = sgplvmCreate_tr(m,void,options)
...
if(isfield(options,'kernelFconstraints')&&~isempty(options.kernelFconstraints)) %%F-norm harmonization
    model.kernelFconstraints = options.kernelFconstraints;
    model.mu = options.mu;
    model.gamma1 = options.gamma1;
else
  model.kernelFconstraints = false;
end

if(isfield(options,'kernelSxL21norm')&&~isempty(options.kernelSxL21norm)) %%L21 harmonization
    model.kernelSxL21norm = options.kernelSxL21norm;
    model.mu = options.mu;
    model.gamma1 = options.gamma1;
else
  model.kernelSxL21norm = false;
end

if(isfield(options,'kernelSxtrace')&&~isempty(options.kernelSxtrace)) %%trace harmonization
    model.kernelSxtrace = options.kernelSxtrace;
    model.mu = options.mu;
    model.gamma1 = options.gamma1;
else
  model.kernelSxtrace = false;
end


% force kernel computation
initParams = sgplvmExtractParam_tr(model);
model = sgplvmExpandParam_tr(model,initParams);
